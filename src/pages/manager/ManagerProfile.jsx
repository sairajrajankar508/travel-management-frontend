...
const ManagerProfile = () => {
  ...
  const fetchProfile = () => {
    apiClient.get("/manager/profile").then((res) => {
      const data = res.data;
      setProfile(data);
  ...
  const handleUpdate = () => {
    const promise = apiClient.put("/manager/profile/update", {
      name: form.name,
      email: form.email,
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-white rounded-3xl shadow-lg border border-slate-200 p-6">
          <div className="flex items-center gap-4 mb-6">
            <div className="p-3 rounded-2xl bg-purple-100"><User className="text-4xl text-purple-600" /></div>
            <div>
              <h1 className="text-2xl font-bold text-slate-800">{profile?.name || "Manager"}</h1>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 min-w-0 *:min-w-0 *:wrap-break-word">
            <div className="bg-slate-50 rounded-2xl p-4">
              ...
              <p className="font-semibold text-slate-800">{profile?.department || "Not assigned"}</p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4">
        ...
        <div className="flex justify-end">
          <button onClick={handleUpdate} className="px-8 py-3 rounded-2xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm flex items-center gap-2 transition">
